...
En general, las expresiones aritméticas pueden estar constituidas por muchas constantes, variables, operadoresy paréntesis. ¿En qué orden se ejecutan las operaciones? Por ejemplo, en la siguiente sentencia: 

```c++
tempPromedio = PUNTO_CONGELACION + PUNTO_EBULLICION / 2.0;
```
...
```
En este caso, se evalua primero la expresion entre parentesis, déspues se divide la suma entre `float1` y finalmente se multiplica por 3.0 el resultado. 
